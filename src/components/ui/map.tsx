...
function MapControls({
  ...
  const handleLocate = useCallback(() => {
    ...
    if (!("geolocation" in navigator)) {
      console.error("Geolocation is not supported by this browser/environment.");
...
> = {
  ...
  /**
   * MapLibre clusterProperties: custom aggregation expressions applied while clustering.
   * Use this to sum a numeric property across clustered features, e.g.:
   * `{ restaurant_count: ["+", ["get", "restaurant_count"]] }`
   */
  clusterProperties?: object;
  /**
   * Name of the feature property to display as the cluster count label.
   * Defaults to MapLibre's built-in "point_count_abbreviated".
   */
  countField?: string;
};
...
>({
  ...
  useEffect(() => {
    ...
    map.addLayer({
      ...
      filter: ["has", "point_count"],
      layout: {
        "text-field": countField ? `{${countField}}` : "{point_count_abbreviated}",
        "text-font": ["Open Sans"],
        "text-size": 12,
      },
      paint: {
